"""Stage B 训练入口:高分辨率(384x512)+ 逐头类别权重 + per-class recall 指标。

不改动 v1 的 train.py;本模块 = train.py 原版 + 上述增强,配置用 config_hi.py。
用法:python -m shoplift.models.person_attribute.train_hi \\
...
from shoplift.models.person_attribute.labels import HEAD_SPECS, HEAD_SPECS_BY_NAME
...
def train(config: TrainConfig, *, resume: Path | None = None) -> None:
    ...
    class_weights = _validate_class_weights(config.loss_class_weights)
    ...
    for epoch in range(1, config.optimizer.epochs + 1):
        ...
        model.train()
        train_metrics = _run_epoch(model, train_loader, optimizer=optimizer, class_weights=class_weights)
        ...
        if val_loader is not None:
            model.eval()
            val_metrics = _run_epoch(model, val_loader, optimizer=None, class_weights=class_weights)
    ...
    if test_loader is not None:
        model.eval()
        test_metrics = _run_epoch(model, test_loader, optimizer=None, class_weights=class_weights)
...
def _validate_class_weights(class_weights: Mapping[str, tuple[float, ...]]) -> dict[str, tuple[float, ...]]:
    validated: dict[str, tuple[float, ...]] = {}
    for head, weights in class_weights.items():
        spec = HEAD_SPECS_BY_NAME.get(head)
        if spec is None:
            raise ValueError(f"unknown head in loss.class_weights: {head}")
        if len(weights) != spec.num_classes:
            raise ValueError(
                f"loss.class_weights[{head}] must have {spec.num_classes} weights "
                f"(per {spec.labels}), got {len(weights)}"
            )
        validated[head] = tuple(float(value) for value in weights)
    return validated


def _accumulate_confusion(matrix: list[list[int]], target: Any, pred: Any) -> None:
    true_labels = target.reshape([-1]).tolist()
    pred_labels = pred.reshape([-1]).tolist()
...
def _run_epoch(
    ...
    *,
    optimizer: Any | None,
    class_weights: Mapping[str, tuple[float, ...]] | None = None,
) -> dict[str, Any]:
    paddle = require_paddle()
    total_loss = 0.0
    total_samples = 0
    ...
    confusion = {
        spec.name: [[0] * spec.num_classes for _ in range(spec.num_classes)]
    ...
    for images, labels in loader:
        ...
        for spec in HEAD_SPECS:
            ...
            weights = class_weights.get(spec.name)
            if weights is not None:
                loss = paddle.nn.functional.cross_entropy(
                    logits[spec.name],
                    target,
                    weight=paddle.to_tensor(list(weights), dtype="float32"),
                )
            else:
            ...
            _accumulate_confusion(confusion[spec.name], target, pred)
        loss = sum(losses)
    ...
    for spec in HEAD_SPECS:
        matrix = confusion[spec.name]
        total = sum(sum(row) for row in matrix)
        correct = sum(matrix[index][index] for index in range(spec.num_classes))
        accuracies[spec.name] = correct / total if total else 0.0
        recall: dict[str, float] = {}
        precision: dict[str, float] = {}
        for index, label in enumerate(spec.labels):
            true_total = sum(matrix[index])
            pred_total = sum(matrix[row][index] for row in range(spec.num_classes))
            tp = matrix[index][index]
            recall[label] = tp / true_total if true_total else 0.0
            precision[label] = tp / pred_total if pred_total else 0.0
        per_class[spec.name] = {"recall": recall, "precision": precision}
    return {
