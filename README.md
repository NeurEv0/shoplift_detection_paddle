...
人员检测/跟踪门控和姿态关键点/骨架可以通过视频专用入口抽帧推理，并保留可视化视频和抽样帧图片。默认测试视频目录为 `datasets/test_videos`，默认每 30 帧推理一次：
...
```powershell
python -m shoplift.cli.video_infer_visualize --input path/to/videos --output outputs/inference_visualization/my_batch --frame-stride 30
python -m shoplift.cli.video_infer_visualize --input path/to/video.mp4 --output outputs/inference_visualization/my_video --frame-stride 15
```
